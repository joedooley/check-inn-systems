...
DB_NAME="${1-gravityview_math_test}"
DB_USER="${2-root}"
DB_PASS="${3-root}"
DB_HOST="${4-localhost}"
WP_VERSION="${5-latest}"
...
print_gv_math_help() {
    ...
    echo "example using remote .zip: $0 gravityview_math_test root root localhost latest http://example.com/path/to/gravityforms.zip"
    echo "example using local path: $0 gravityview_math_test root root localhost latest ../gravityforms/"
...
install_gravityview(){
	curl -L https://github.com/gravityview/GravityView/archive/develop.tar.gz --output /tmp/gravityview.tar.gz --silent

	mkdir -p $PWD/tmp/gravityview
	tar --strip-components=1 -zxf /tmp/gravityview.tar.gz -C $PWD/tmp/gravityview
}
...
install_db() {
    ...
    if mysqlshow "${DB_NAME}" --user="$DB_USER" --password="$DB_PASS"$EXTRA &> /dev/null
    then
