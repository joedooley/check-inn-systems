...
class GravityView_Ratings_Reviews_Helper {
	...
	public static function get_the_reviews_link() {
		...
		if ( $post && ! is_wp_error( $post ) ) {
			...
			$return_link = gravityview_get_link(
				...
				self::get_reviews_number_text( $post, $fs['no_comment_text'], $fs['one_comment_text'], $fs['more_comments_text'] ),
				array(
		...
		if ( $post_bridge->name === get_post_type( $bridge_post_id ) ) {
			...
			if ( is_admin() ) {
				...
				if( ! empty( $_POST['redirect_to'] ) ) {
					...
					if( 'get_comment_link' === current_filter() ) {
	...
	public static function get_vote_rating( $args = array() ) {
		...
		if ( true === GravityView_Ratings_Reviews_Helper::is_ratings_allowed( null, null, $gravityview_view ) ){
			...
			$output = '';
			if ( $r['clickable'] ) {
				$output_wrapper = '<span class="gv-vote-rating gv-vote-rate-holder">';
				$vote_tag = 'a';
			} else {
	...
	/**
	 * Outputs a HTML element with the vote in integers
	 *
	 * @since 1.3
	 *
	 ...
	 * @return bool
	 */
	public static function is_user_allowed_to_leave_review( $post_bridge, $review_author = '', $review_author_email = '', $reviewdata = array() ) {
	...
	public static function is_ratings_allowed( $form = null, $entry = null, $view = null ){
		...
		/**
		 * @filter `gv_ratings_reviews_ratings_allowed` Allow rating an entry for this View?
	...
	public static function is_reviews_allowed( $form = null, $entry = null, $view = null ){
		 ...
		 * @param  object|array|int  $view  GravityView Object or ID
		 */
		return apply_filters( 'gv_ratings_reviews_reviews_allowed', true, $form, $entry, $view );
	}
