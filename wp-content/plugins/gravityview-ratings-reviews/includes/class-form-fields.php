...
class GravityView_Ratings_Reviews_Form_Fields extends GravityView_Ratings_Reviews_Component {
	...
	public function load() {
		...
		add_filter( 'gform_post_update_entry', array( $this, 'new_entry' ), 15 );
		add_filter( 'gform_post_submission', array( $this, 'new_entry' ), 15 );
	...
	public function new_entry( $entry ){
		$metas = GravityView_Ratings_Reviews_Helper::get_ratings_detailed( null, $entry['id'] );

		foreach ( $metas as $meta_key => $meta_value ) {
			gform_update_meta( $entry['id'], 'gravityview_ratings_' . $meta_key, $meta_value );
		}
	}

	/**
	 * Allow the ordering for Rating Fields by adding the Rating fields on the to 'gform_entry_meta'
	 *
	 * @since  0.1.1
	...
	private static function is_gravityview_screen() {
		$is_gravityview = ( function_exists('gravityview_is_admin_page') && gravityview_is_admin_page() ) && ! self::is_export_screen();
