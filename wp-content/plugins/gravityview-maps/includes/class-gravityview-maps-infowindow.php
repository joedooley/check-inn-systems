...
/**
 * Class GravityView_Maps_InfoWindow
 */
class GravityView_Maps_InfoWindow {
	...
	/**
	 * GravityView_Maps_InfoWindow constructor.
	 *
	...
	/**
	 * Set the Infowindow content variables according to the Map settings
	 ...
	 * @since TODO
	 *
	 * @param GravityView_View $view Currently-displayed View
	...
	function get_marker_content( $view, $entry, $entry_url = '' ) {
		...
		// Replace gravity forms & Gravityview merge tags from the infowindow markup
		if( method_exists( 'GravityView_Merge_Tags', 'replace_variables' ) ) {
			$content = GravityView_Merge_Tags::replace_variables( $content, $view->getForm(), $entry );
		}

		$backup_entry = $view->getCurrentEntry( $entry );
		...
		$view->setCurrentEntry( $backup_entry );
