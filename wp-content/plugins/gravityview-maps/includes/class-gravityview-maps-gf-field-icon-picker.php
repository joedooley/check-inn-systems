...
class GravityView_Maps_GF_Field_Icon_Picker extends GF_Field {

	public $type = 'gvmaps_icon_picker';

	public function get_form_editor_field_title() {
		return __( 'Map Icon', 'gravityview-maps' );
	}
	...
	public function validate( $value, $form ) {
		...
		if ( empty( $value ) ) {
			...
			if ( $this->isRequired ) {
				...
				$this->validation_message = empty( $this->errorMessage ) ? __( 'This field is required.', 'gravityview-maps' ) : $this->errorMessage;
			}
		}

		if ( ! empty( $value ) && ! GFCommon::is_valid_url( $value ) ) {
			$this->failed_validation  = true;
			$this->validation_message = empty( $this->errorMessage ) ? __( 'Please select a valid icon.', 'gravityview-maps' ) : $this->errorMessage;
		}
	...
	public function get_default_icon() {
		...
		return $gravityview_maps->component_instances['available-icons']->get_default_icon_url();
	}
	...
	/**
	 * Return value for the entries list (GF Admin)
	 *
	 * @param $value
	 * @param $entry
	 * @param $field_id
