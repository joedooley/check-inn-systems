<?php
/**
 * Fetch the available map icons on plugin, theme or custom
 *
 ...
 * @since 1.0.0
 */
class GravityView_Maps_Available_Icons extends GravityView_Maps_Component {
	...
	public function get_default_icon_url() {
		...
		$default_icon = $this->loader->url .'assets/img/google-default.png';
	...
	public function render_available_icons( $part = 'available-icons', $field_id = 0 ) {
		...
		$sections['default'] = __( 'Default', 'gravityview-maps' );
		...
		if( !empty( $icons['theme'] ) ) {
			$sections['theme'] = __( 'Theme Map Icons', 'gravityview-maps' );
		}
		...
		$sections['plugin'] = __( 'GravityView Map Icons', 'gravityview-maps' );
