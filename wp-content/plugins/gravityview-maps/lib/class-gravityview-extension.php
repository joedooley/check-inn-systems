...
abstract class GravityView_Extension {
	...
	protected function is_extension_supported() {
		...
		if( !class_exists( 'GravityView_Plugin' ) ) {
			...
			$message = sprintf( __('Could not activate the %s Extension; GravityView is not active.', 'gravityview-maps'), $this->_title );

		} else if( false === version_compare(GravityView_Plugin::version, $this->_min_gravityview_version , ">=") ) {

			$message = sprintf( __('The %s Extension requires GravityView Version %s or newer.', 'gravityview-maps' ), $this->_title, '<tt>'.$this->_min_gravityview_version.'</tt>' );

		} else if( isset( $this->_min_php_version ) && false === version_compare( phpversion(), $this->_min_php_version , ">=") ) {

			$message = sprintf( __('The %s Extension requires PHP Version %s or newer. Please ask your host to upgrade your server\'s PHP.', 'gravityview-maps' ), $this->_title, '<tt>'.$this->_min_php_version.'</tt>' );
